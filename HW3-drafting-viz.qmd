...
2.  What ggplot extension tools / packages do you need to use to build your visualizations? Are there any that we haven't covered in class that you'll be learning how to use for your visualizations?
    1.  I have been working with `geom_segment()` and `geom_labelsegment()` extensively for my circular visualizations, which I do not think we used in class. I have been also heavily editing the `theme()` arguments as well as `scale_fill_brewer()` and `scale_color_manual()` to establish the theme elements that I would like to be across my visualizations. Additionally I used the `geom_sf()` package in order to construct my map of Germany using `ggplot()`.
